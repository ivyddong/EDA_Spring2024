...
#ggplot(Dataset(the data we want to use), asethetics(how we gonna realize this), x = the specific subdataset) + the type of geometry we want to display
```
...
# Generally we use the binwidth between 5 to 10
ggplot(USGS.flow.data) +
...
#
ggplot(USGS.flow.data) +
# compare with **geom_frepoly**: geom_histogram(aes(x = gage.height.mean), bins = 50) + 
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "darkgray") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) + # lty = linetype, 2 for dashed line
