...
irradiance.regression <- lm(
  ...
  irradianceWater ~ depth) # The formula irradianceWater ~ depth specifies that we want to model the variable irradianceWater as a function of depth.
summary(irradiance.regression)

# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
# This line performs a correlation test between the variables irradianceWater and depth.
# It calculates Pearson's correlation coefficient and conducts a hypothesis test to determine if the correlation is statistically significant.
# The result includes correlation coefficient, p-value, and confidence interval for the correlation coefficient.
```
