...
```{r}
cars$speed # These numbers are shown in simple vectors
```
...
# is it possible to extract some specific speed according to distance or reversely?
```
...
```{r}
car_dist <- cars$dist
class(car_dist) # Notice that this is a numeric value

#Option 1 (Do it directly)
