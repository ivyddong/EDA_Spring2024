...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # We dont respectivelt mention "nrow" or "ncol". The matrix would be the same.
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # If using "byrow = FALSE", the matrix would be the same with matrix1
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "quatro", "cinco"), # column
                                  c("un", "deux", "trois", "quatre"))) # row
...
```{r DataExercise-1a}
F_HighTemp <- c(52, 34, 41, 29, 56, 65, 69, 69, 66, 62)
F_LowTemp <- c(21, 18, 18, 29, 46, 57, 58, 58, 51, 42)

F_Temps <- data.frame(F_HighTemp, F_LowTemp)
```
...
C_HighTemp <- F_to_C(F_Temps$F_HighTemp)
