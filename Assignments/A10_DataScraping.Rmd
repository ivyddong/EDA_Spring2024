...
```{r}
#Create our modified scraping function
scrape.it2 <- function(the_year2, the_pwsid2){
  # Construct the scraping web address
  the_base_url2 <- 'https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid='
  the_scrape_url2 <- paste0(the_base_url2, the_pwsid2, '&year=', the_year2)
  
  #Retrieve the website contents 
  the_website2 <- read_html(the_scrape_url2)
  
  #Set the element address variables
  the_WaterSystem_tag2 <- 'div+ table tr:nth-child(1) td:nth-child(2)'
  the_PWSID_tag2 <- 'td tr:nth-child(1) td:nth-child(5)'
  the_Ownership_tag2 <- 'div+ table tr:nth-child(2) td:nth-child(4)'
  the_MaxWithdraws_tag2 <- 'th~ td+ td'
  
  #Scrape the data items
  the_WaterSystem2 <- the_website2 %>% html_nodes(the_WaterSystem_tag2) %>% html_text()
  the_PWSID2 <- the_website2 %>% html_nodes(the_PWSID_tag2) %>%  html_text()
  the_Ownership2 <- the_website2 %>% html_nodes(the_Ownership_tag2) %>% html_text()
  the_MaxWithdraws2 <- the_website2 %>% html_nodes(the_MaxWithdraws_tag2) %>% html_text()
  
  #Convert to a dataframe
  df_withdrawals2 <- data.frame("Year" = rep(the_year2,12)) %>% 
    mutate(WaterSystem = !!the_WaterSystem2,
           PWSID = !!the_PWSID2,
           Ownership = !!the_Ownership2,
           MaxDayUse = as.numeric(the_MaxWithdraws2),
           Date = my(paste(LWSP_Month_numeric,"-",Year)))  
...
Durham_2016 <- scrape.it2(2016, '03-32-010')
```
...
#9 
#Set the inputs to scrape years 2015 to 2021 for Asheville
...
> Answer:
> With the year of 2015 as the turning point, before 2015, the water usage in Asheville slightly decreased; after 2015, it increased with a relatively rate.
