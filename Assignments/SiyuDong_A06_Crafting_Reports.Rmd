...
```{r project.setup, message = FALSE, error = FALSE, warning = FALSE, echo = TRUE}
#Import libraries
...
```{r, echo=FALSE, fig.align='left', fig.height=3, fig.cap="2018 Ozone values by month", warning=FALSE}
ozone_2018 <- epa_data %>%
  filter(year(Date) == 2018) %>%
  mutate(Month = factor(month(Date), labels = month.abb)) %>%
  ggplot(aes(x = Month, y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Concentration", title = "Monthly Ozone concentrations - 2018")
print(ozone_2018)
```

```{r, echo=FALSE, fig.align='left', fig.height=3, fig.cap="2019 Ozone values by month", warning=FALSE}
ozone_2019 <- epa_data %>%
  filter(year(Date) == 2019) %>%
  mutate(Month = factor(month(Date), labels = month.abb)) %>%
  ggplot(aes(x = Month, y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Concentration", title = "Monthly Ozone concentrations - 2019")
print(ozone_2019)
```
...
## Git Repository
https://github.com/ivyddong/EDA_Spring2024