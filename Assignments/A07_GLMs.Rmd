...
Lake.Chem.Raw <- read.csv(here("Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"), stringsAsFactors = TRUE)
...
step(TPAIC)
# According to the outcome, none of the predictors should be deleted and the formula goes like: lm(formula = temperature_C ~ year4 + daynum + depth, data = Lake.Chem.July)
...
Depth_Temp_Plot <- ggplot(Lake.Chem.July, aes(x = depth, y = temperature_C, color = lakename)) +
  geom_point(alpha = 0.5) +  # Making points 50% transparent
