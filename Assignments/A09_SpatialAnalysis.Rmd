...
#24 Plot
# Create a map showing all Nebraska counties
map_nebraska_counties <- mapview(County_Nebraska_UTM, col.regions = "lightblue")

# Add the selected county (Lancaster) to the map
map_with_lancaster <- map_nebraska_counties + County_Lancaster

# Add the gage sites within Lancaster county to the map
map_with_gage_sites <- map_with_lancaster + NationalWater_gage_Lancaster
...
# Plot all Nebraska counties
nebraska_counties_plot <- ggplot() +
  geom_sf(data = County_Nebraska_UTM, fill = "lightgrey", color = "black") +
  labs(title = "Nebraska Counties")

# Plot the selected county (Lancaster)
selected_county_plot <- nebraska_counties_plot +
  geom_sf(data = County_Lancaster, fill = "blue", color = "black") +
  labs(title = "Nebraska Counties with Selected County (Lancaster)")

# Plot the gage sites in the selected county
final_plot <- selected_county_plot +
  geom_sf(data = Gages_In_Lancaster, color = "red") +
  labs(title = "Nebraska Counties with Selected County (Lancaster) and Gage Sites")
